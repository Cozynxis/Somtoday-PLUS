...
var defaults={
 ...
 highlightToday:true,hideCancelledLessons:false,fadePastLessons:false,emphasizeInsufficientGrades:false,privacyBlurNames:false,privacyBlurSchedule:false,
 openLinksNewTab:false,confirmExternalLinks:false,keyboardShortcuts:true,escapeClosesDialogs:true,reduceVisualNoise:false,dimSecondaryText:false
};
var api={defaults:defaults};
api.get=async function(){var saved=await chrome.storage.local.get(defaults);return Object.assign({},defaults,saved);};
...
api.exportSettings=async function(){var data=await api.get();return JSON.stringify({app:'Somtoday PLUS+',version:1,exportedAt:new Date().toISOString(),settings:data},null,2);};
api.importSettings=async function(raw){var parsed=typeof raw==='string'?JSON.parse(raw):raw;if(!parsed||typeof parsed!=='object')throw new Error('Ongeldig bestand');var incoming=parsed.settings||parsed,safe={};Object.keys(defaults).forEach(function(key){if(Object.prototype.hasOwnProperty.call(incoming,key))safe[key]=incoming[key];});await api.set(safe);return safe;};
api.export=api.exportSettings;api.import=api.importSettings;
